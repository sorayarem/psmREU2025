...
plt.plot(data['year'], data['isleAuHautIso'], label='shell record', linestyle='-', color='#008080')
...
df = pd.DataFrame({ 'year': data['year'], 'observed': data['isleAuHautIso'], 'pseudocarb': data['pseudocarbonate']})
...
rmse = np.sqrt(((data['isleAuHautIso'] - data['pseudocarbonate']) ** 2).mean())
print("RMSE:", rmse)

## finding null-model root mean squared error
nrmse = np.sqrt(((data['isleAuHautIso'] - 0) ** 2).mean())
print("NRMSE:", nrmse)
...
dT = ((1+rA*rB)/(1-rA*rB))
nEff = (data['isleAuHautIso'].shape)/dT
print("N-eff:", nEff)
print("df Used:", min(nEff, data['isleAuHautIso'].shape[0] - 1))
