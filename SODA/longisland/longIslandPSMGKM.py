...
## loading in the data
sodaRaw = pd.read_csv("./csv_clean/longIslandCSV.csv")
...
plt.plot(data['year'], data['longIslandIso'], label='shell record', linestyle='-', color='#008080')
...
rmse = np.sqrt(((data['longIslandIso'] - data['pseudocarbonate']) ** 2).mean())
print("RMSE:", rmse)

## finding null-model root mean squared error
nrmse = np.sqrt(((data['longIslandIso'] - 0) ** 2).mean())
print("NRMSE:", nrmse)
...
dT = ((1+rA*rB)/(1-rA*rB))
nEff = (data['longIslandIso'].shape)/dT
print("N-eff:", nEff)
print("df Used:", min(nEff, data['longIslandIso'].shape[0] - 1))
