...
dsTemp = xr.open_dataset("./VIKING20X/selections/tempLONGISLAND.nc")
dsSaline = xr.open_dataset("./VIKING20X/selections/saltLONGISLAND.nc")
...
spatialMeanTemp = spatialMeanTemp.sel(deptht = 16, method = 'nearest')
spatialMeanSaline = dsSaline.mean(dim=['y', 'x'])
spatialMeanSaline = spatialMeanSaline.sel(deptht = 16, method = 'nearest')
...
plt.title('Long Island - d18O-carb and d18O-pseudo Over Time')
plt.legend()
plt.grid(True)
