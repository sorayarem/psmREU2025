...
## loading in the data
dsTemp = xr.open_dataset("./iCESM/selections/tempSEGUIN.nc")
dsSaline = xr.open_dataset("./iCESM/selections/saltSEGUIN.nc")
...
spatialMeanTemp = spatialMeanTemp.sel(z_t = 0, method = 'nearest')
spatialMeanSaline = dsSaline.mean(dim=['nlat', 'nlon'])
spatialMeanSaline = spatialMeanSaline.sel(z_t = 0, method = 'nearest')

## getting the mean for each overall year
print("Computing means...")
vikingTempAnnual = spatialMeanTemp.groupby('year').mean('time')
vikingSalineAnnual = spatialMeanSaline.groupby('year').mean('time')

vikingTempExpert = spatialMeanTemp.groupby('expertYear').mean('time')
vikingSalineExpert = spatialMeanSaline.groupby('expertYear').mean('time')
...
resultAnomalies['temp'] = vikingTempExpert['TEMP'] if season else vikingTempAnnual['TEMP']
resultAnomalies['salt'] = vikingSalineExpert['SALT'] if season else vikingSalineAnnual['SALT']
